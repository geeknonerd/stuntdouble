...
- 上游 HTTP（`ctx.http.get` 与支持 Range 透传的流式 `ctx.http.pipe` 已实现，`ctx.http.request` 待后续切片）以及 `ctx.file` 读取、文件流与本地文件 Range 响应；上传待后续切片。
- 唯一静态文件根，并阻止路径穿越。
