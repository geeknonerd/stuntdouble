...
- 数据预算 `files.upload_max_bytes`：只计 part 的 content bytes，文件与非文件字段都计，不含 multipart framing；默认 20 MiB（`src/config.rs:42-45`、`src/config.rs:674-676`、`src/files.rs:316-320`）。
- 原始请求流上限：计整个 multipart body stream 的 wire bytes，包含 boundary、part header、CRLF 等 framing；取值是 `files.upload_max_bytes` 加一个固定的 1 MiB allowance（`src/server.rs:342-353`、`docs/contracts/config.md:79`、`SECURITY.md:23`）。
...
数据预算的权威定义是 `files.upload_max_bytes`。它要求正整数；0、负数和非整数在加载配置时就会被拒绝（`src/config.rs:555-578`、`docs/contracts/config.md:29`）。解析器对每一个 `field.chunk()` 累加 `total_bytes`，一旦超过预算立即返回 `ParseFailure::TooLarge`；非文件字段同样累加，只是不落盘（`src/files.rs:356-373`）。解析结束后的 `UploadStore.total_bytes` 也是这份账目（`src/files.rs:397-400`）。
...
当前实现见 `src/server.rs:342-353`；`MULTIPART_FRAMING_ALLOWANCE` 是固定的 1 MiB 常量（`src/server.rs:576-577`）。公开契约把它描述为“整个 multipart body stream，including framing，受 `upload_max_bytes` 加 1 MiB 约束”，数据预算本身仍不含 framing（`docs/contracts/config.md:79`、`SECURITY.md:23`）。
...
当前实现见 `src/server.rs:152-163`。这里有两个必须同时成立的事实：

- `Content-Length` 预检比较的是 `max_bytes + MULTIPART_FRAMING_ALLOWANCE`，因为它判断的是 wire bytes；声明长度超过 raw ceiling 时，可以在读取 body 之前直接返回 413（`src/server.rs:155-161`）。
- `parse_multipart` 收到的仍然只有 `max_bytes`，所以流式账目不会被 1 MiB allowance 污染（`src/server.rs:163`、`src/files.rs:321-324`）。

没有 `Content-Length` 或客户端声明值不可信时，流式计数仍然有效；`multipart_limit_accepts_the_cap_and_rejects_precheck_chunked_and_form_fields` 同时覆盖了预算命中、超限、无 Content-Length 的 chunked 请求、非文件字段和声明长度预检（`tests/cli.rs:1808-1892`）。框架层 raw stream 被超过时，axum 把 multipart 解析失败映射为 413，本地分类代码据此返回同一个稳定类别 `upload_too_large`（`src/files.rs:421-430`、`src/files.rs:267-273`、`src/server.rs:529-544`）。这意味着“内容超预算”和“framing 把原始流顶穿”在客户端上共享 413 类别，但触发它们的账目不同，日志里的 `upload.total_bytes` 记录的仍是已读内容字节（`src/server.rs:163-181`）。

### 3. 共享 Router 默认值覆盖 multipart 原始流，应用层再按路径收紧

当前 Router 只有一个 `fallback(any(handle))`，业务路由匹配和 multipart/非 multipart 分支都在 handler 内完成（`src/server.rs:350-352`、`src/server.rs:142-213`）。因此 Router 层只能有一个共享的默认请求体值，不能因为“这条路由是 multipart、那条不是”自动切换两个上限。
...
当前实现见 `src/server.rs:189-210`，`NON_MULTIPART_BODY_LIMIT` 是 `2 * 1024 * 1024`（`src/server.rs:573-574`）。这就是“应用层重新收紧”：在默认 20 MiB 配置下，框架共享值（21 MiB）比非 multipart 的 2 MiB 更宽，其语义是 multipart 的 raw ceiling；真正走非 multipart 分支时，读取上限重新变成 2 MiB。
...
`MULTIPART_FRAMING_ALLOWANCE` 固定为 1 MiB（`src/server.rs:576-577`），公开契约也把它写成固定 allowance（`docs/contracts/config.md:79`）。它不应映射成用户配置，原因有三点：
...
multipart 的解析失败走项目 JSON error envelope，包含 `request_id` 和错误类别（`src/server.rs:529-544`、`docs/contracts/config.md:79`）。非 multipart 超过 2 MiB 时走 `Handled::BodyTooLarge`，返回 413、空 body，不返回 multipart/script 的 JSON envelope（`src/server.rs:469-477`）。代码注释明确把它称为 pre-T12 行为，回归测试 `non_multipart_body_limit_stays_bounded` 构造 2 MiB + 1 字节请求并断言 413 且 body 为空（`tests/cli.rs:2128-2155`）。

这是有意的兼容决定，不是遗漏：T12 首版实现曾把非 multipart 超限改成新的 `request_body_too_large` JSON 公共错误类，评审判定那是超出 spec 的范围蔓延，随后回退（session history）。引入 multipart 的新错误 envelope 时，不应顺手改变原有非 multipart 客户端的响应形状。内部日志仍可使用 `request_body_too_large` 作为 class（`src/server.rs:473`），但客户端看到的是空 body；文档也明确区分这两类失败（`docs/contracts/config.md:79`）。

### 6. 测试必须同时覆盖两本账和两条路径

T12 当前回归覆盖了以下边界，建议后续改动沿用同一组形状：

- **数据预算恰好命中**：配置 64 字节，提交 64 字节 part content，返回 200。测试 helper 会加入 boundary、Content-Disposition、Content-Type 和 CRLF，因此实际 wire bytes 明显大于 64；能够成功依赖的正是 `64 + 1 MiB` 的原始流 allowance（`tests/cli.rs:352-373`、`tests/cli.rs:1817-1831`、`tests/cli.rs:1868-1871`）。
- **内容超预算**：65 字节 part content、无 Content-Length 的 chunked 请求、65 字节非文件字段，均在流式计数阶段返回 413 `upload_too_large`（`tests/cli.rs:1832-1859`、`tests/cli.rs:1872-1879`）。
- **声明长度预检**：声明 `limit + 1 MiB + 1`，在读取 body 之前返回 413 `upload_too_large`（`tests/cli.rs:1860-1865`、`tests/cli.rs:1872-1879`）。
- **只攻击 framing**：配置只允许 64 字节内容，但发送一个超长 part name，让 chunked multipart 的 wire bytes 超过 allowance；必须 413 `upload_too_large`，证明原始流闸门不是装饰（`chunked_multipart_framing_is_bounded`，`tests/cli.rs:2157-2183`）。
- **非 multipart 兼容**：2 MiB + 1 字节返回空 body 413（`tests/cli.rs:2128-2155`）。
...
- 数据预算 = 20 MiB（`src/config.rs:674-676`、`docs/contracts/config.md:29`）。
- 原始流上限 = 20 MiB + 1 MiB = 21 MiB（`src/server.rs:342-346`、`docs/contracts/config.md:79`）。
...
`multipart_limit_accepts_the_cap_and_rejects_precheck_chunked_and_form_fields` 把 `upload_max_bytes` 设为 64（`tests/cli.rs:1817-1819`）：

- 64 字节文件返回 200（`tests/cli.rs:1821-1831`、`tests/cli.rs:1868-1871`）。
- 65 字节文件、65 字节 chunked 请求、65 字节非文件字段都返回 413 `upload_too_large`（`tests/cli.rs:1832-1859`、`tests/cli.rs:1872-1879`）。
- 一个声明长度为 `64 + 1 MiB + 1` 的 multipart 请求由预检直接返回 413，不需要等待 body（`tests/cli.rs:1860-1865`、`tests/cli.rs:1872-1879`）。
...
非 multipart 请求不进入 `parse_multipart`，而是执行 `axum::body::to_bytes(..., NON_MULTIPART_BODY_LIMIT)`（`src/server.rs:189-210`）。即使共享的 Router 默认值为了 multipart 放宽到 20 MiB + 1 MiB，非 multipart 仍在 2 MiB 处被截断：

- `NON_MULTIPART_BODY_LIMIT = 2 * 1024 * 1024`（`src/server.rs:573-574`）。
- 2 MiB + 1 字节请求得到 413 空 body，不返回 JSON error envelope（`src/server.rs:469-477`、`tests/cli.rs:2128-2155`）。
...
- 回归测试：`tests/cli.rs:1808-1892`（预算命中、超限、chunked、普通字段、声明长度预检）与 `tests/cli.rs:2157-2183`（纯 framing 超限）；非 multipart 兼容见 `tests/cli.rs:2128-2155`。
