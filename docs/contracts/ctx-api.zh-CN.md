...
- **状态**：v1 切片（T1–T8）已公开并冻结；本页描述第一切片子集
- **适用**：`apiVersion` 1
...
| 文件 | `ctx.file.readText` / `ctx.file.readBytes` / `ctx.file.stream` | pending |
...
以下能力属于更长的 v1 计划，但不在本切片中。它们有意不出现在 `ctx` 与 `types/ctx-api-v1.d.ts` 中；调用不存在的成员是普通脚本错误，映射为 500 `script_error`。

- `ctx.request.bodyBytes`
- `ctx.http.request` 与 `ctx.http.get` 的 retry/backoff 选项
- `ctx.file.readText`、`ctx.file.readBytes` 与 `ctx.file.stream`
- `ctx.local`
