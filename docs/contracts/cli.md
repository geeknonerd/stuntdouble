...
Starts the mock server binding to the configured address. Matched Routes execute their JavaScript in the embedded Boa runtime with a host-injected `ctx`, including allowlisted `ctx.http.get` calls, streaming `ctx.http.pipe` calls, and rooted `ctx.file` reads with streamed local file responses, and request-scoped multipart uploads exposed through `ctx.request.files`; unmatched Routes answer 404 `not_found`. A matched multipart request is parsed before the script runs: malformed multipart answers 400 `invalid_multipart`, file and form-field data over `files.upload_max_bytes` answers 413 `upload_too_large`, and a non-multipart body over the 2 MiB bound keeps its bounded 413 response. A matched route whose body or multipart parse does not finish within `server.request_timeout_ms` answers 408 `request_timeout`; a request head that never completes is closed without a Response and logged as `request_head_timeout`, because no Response can be written yet. Script failures answer 500 `script_error` or `script_no_response`; an uncaught upstream transport failure answers 502 `upstream_unreachable`.

`--verbose` attaches a `detail` field to engine-generated JSON error bodies, including multipart 400/413, the request-body 408, and engine 500/502 responses. The value is a stable failure class, for example `script execution failed`, `script exceeded the configured timeout`, `script worker capacity exhausted`, `upstream transport failure: timeout`, `upstream transport failure: dns`, or `upstream transport failure: transport`. It never contains stack traces, script messages, upstream bodies, hostnames, IP addresses, or URLs. Without the flag, error bodies contain only `error` and `request_id`. Use it for local diagnosis only; do not enable it in shared environments. A 404 `not_found` response never carries `detail`; the non-multipart 413 response has an empty body and no `detail`.
...
- `upload` is `{files, total_bytes, error}`. `files` counts stored file parts; `total_bytes` counts file and non-file field data counted before parsing finished; framing bytes are excluded. `error` is `invalid_multipart`, `upload_too_large`, `upload_io_error`, `request_timeout`, or `null`. A pre-script parsing failure still writes one request line with a stable error class and a `null` `script_duration_ms`. Client filenames and temporary paths never enter the log.
- A request head that never completes writes one connection-level line, `{"error":"request_head_timeout"}`, and no request line, because no request was parsed.
