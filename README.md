...
- Upstream HTTP (`ctx.http.get` plus streaming `ctx.http.pipe` with Range passthrough; `ctx.http.request` pending) and `ctx.file` reads, file streaming, and local file Range responses; uploads are pending.
