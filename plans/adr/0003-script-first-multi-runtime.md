...
- 沙箱安全边界：网络白名单与文件访问见 ADR 0004；超时与内存边界见本文件 T3 修订
- 脚本 API 契约（脚本能读什么、能做什么、怎么返回响应）
- 子进程 vs 嵌入式引擎的最终技术路径（内存硬边界见 #28、孤儿 worker 并发上限见 #29）
...
## 修订（#29）：脚本 worker 并发上限

T3 限定了单个脚本的资源包络，但没有限定被放弃 worker 的数量：每次超时都会在阻塞池里留下一个仍在运行的 worker，持续超时会让 worker 数与内存无界增长（#29）。

- `AppState` 持有一个信号量，槽位数按可用并行度推导、下限 4、上限 16：低核 CI 至少 4 个槽位，多核主机最多 16 个，避免 Boa 上下文无界驻留。
- 每个脚本 worker 在**整个生命周期**持有许可，客户端应答时限到达后仍持有，直到循环次数兜底真正结束 worker。
- 槽位耗尽时新请求**快速失败**，不排队：500 `script_error`，`--verbose` 的 detail 为 `script worker capacity exhausted`。排队只会把请求自己的 deadline 耗在队尾，并放大内存占用。

这不是内存硬边界；#28 的进程隔离仍是长期修复。进程隔离落地后，可由 OS 级 CPU/内存限制取代本修订的并发上限（或保留为进程数上限）。
