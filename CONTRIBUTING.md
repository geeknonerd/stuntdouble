...
The v1 slices through T13 are implemented: `stuntdouble serve` and `stuntdouble validate` run against a TOML configuration, matched routes execute their JavaScript in the embedded Boa runtime with a host-injected `ctx` (including allowlisted `ctx.http.get`, streaming `ctx.http.pipe`, rooted `ctx.file` reads, and request-scoped `ctx.request.files` uploads), and every response carries a `request_id`. Unmatched routes answer 404 `not_found`; script failures answer 500 `script_error` or `script_no_response`; uncaught upstream transport failures answer 502 `upstream_unreachable`. The document manifest and PDF download scenarios from [plans/demo-document-catalog.md](plans/demo-document-catalog.md) run from [demo/](demo/). Local static-file reads and streamed file responses are implemented (T11), multipart uploads are parsed into request-scoped temporary storage (T12), and T13 runs the document demo fixture fully offline: the local manifest reads `files/metadata.json` with `ctx.file.readText`, the local download streams fixture PDFs with `ctx.file.stream`, and the upload route echoes multipart metadata without persisting anything. `server.request_timeout_ms` bounds how long a client may take to finish a request head or body: an unfinished head is closed and logged as `request_head_timeout`, and a matched route's unfinished body answers 408 `request_timeout`. Matched scripts run in a host-owned worker pool of 4–16 slots derived from available parallelism; when every slot is held, a script fails fast with 500 `script_error` instead of queueing, and `--verbose` adds the `script worker capacity exhausted` detail. Product definition, architecture decisions (ADRs), and research notes remain the design source of truth.
